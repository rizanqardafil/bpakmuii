...
class Mynotes extends CI_Controller {
	...
	function add_note($date){
		...
		$data = array(
					'title'	=> 'Tambah Jadwal - '.$site['namaweb'],
					'day'   => $date,
					'mon'   => $this->input->post('mon'),
					'month' => $this->_month($this->input->post('mon')),
					'year'  => $this->input->post('year'),
				);
		$this->load->view('admin/jadwal/add_note', $data);
	}
	...
	function updel_note($date){
		$site = $this->mConfig->list_config();
		$data = array(
					'title'	=> 'Edit/Hapus Jadwal - '.$site['namaweb'],
					'day'   => $date,
					'mon'   => $this->input->post('mon'),
					'month' => $this->_month($this->input->post('mon')),
					'year'  => $this->input->post('year'),
		...
		$this->load->view('admin/jadwal/updel_note', $data);
	}
	...
	function index(){
		redirect('admin/jadwal/mynotes/notes');
	}
	...
	function notes($year = null, $mon = null){
		...
		$data = array(
					'title'	=> 'Jadwal - '.$site['namaweb'],
					'notes' => $this->calendar->generate($year, $mon, $date),
					'year'  => $year,
					'mon'   => $mon,
					'isi'   =>'admin/jadwal/mynotes'
				);
	...
	function _setting(){
		return array(
			'start_day' 		=> 'monday',
			'show_next_prev' 	=> true,
			'next_prev_url' 	=> site_url('admin/jadwal/mynotes'),
			'month_type'   		=> 'long',
            'day_type'     		=> 'short',
			'template' 			=> '{table_open}<table class="date" width="80%">{/table_open}
								   ...
								   {cal_cell_content}<a href="'.site_url('admin/jadwal/mynotes/updel_note').'/{day}" class="act_note" title="Edit/hapus catatan untuk tanggal {day}"><div class="kactive act_note" val="{day}" title="Edit/Hapus catatan untuk tanggal {day}">{day}</div></a><div class="notes">{content}</div></div>{/cal_cell_content}
								   {cal_cell_content_today}<a href="'.site_url('admin/jadwal/mynotes/updel_note').'/{day}" class="act_note" title="Edit/hapus catatan untuk tanggal {day}"><div class="t_kactive" title="Edit/Hapus catatan untuk tanggal {day}">{day}</div></a><div class="t_notes">{content}</div>{/cal_cell_content_today}
								   {cal_cell_no_content}<a href="'.site_url('admin/jadwal/mynotes/add_note').'/{day}" class="act_note" title="Tambah catatan untuk tanggal {day}"><div class="day" title="Tambah catatan untuk tanggal {day}">{day}</div></a>{/cal_cell_no_content}
								   {cal_cell_no_content_today}<a href="'.site_url('admin/jadwal/mynotes/add_note').'/{day}" class="act_note" title="Tambah catatan untuk tanggal {day}"><div class="today" title="Tambah catatan untuk tanggal {day}">{day}</div></a>{/cal_cell_no_content_today}
